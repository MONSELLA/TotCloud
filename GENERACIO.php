<?php
// GENERACIO.php
// Classe per gestionar les Generacions amb paginació i cerca
...
class GENERACIO {
    ...
    // Afegir una nova Generació
    public function addGeneracio($nom) {
        $stmt = $this->conn->prepare("INSERT INTO GENERACIO (nom) VALUES (?)");
        $stmt->bind_param("s", $nom);
        $stmt->execute();
        $stmt->close();
    }

    // Eliminar una Generació
    public function deleteGeneracio($nom) {
        $stmt = $this->conn->prepare("DELETE FROM GENERACIO WHERE nom = ?");
        $stmt->bind_param("s", $nom);
        $stmt->execute();
        $stmt->close();
    }

    // Actualitzar una Generació
    public function updateGeneracio($old_nom, $new_nom) {
        $stmt = $this->conn->prepare("UPDATE GENERACIO SET nom = ? WHERE nom = ?");
        $stmt->bind_param("ss", $new_nom, $old_nom);
        $stmt->execute();
        $stmt->close();
    }

    // Obtenir el codi HTML per mostrar les Generacions amb paginació i cerca
    public function getHTML() {
        ...
        $search_param = "%" . $search . "%";
        $stmt = $this->conn->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM GENERACIO WHERE nom LIKE ? LIMIT ?, ?");
        ...
        $html = '<h2>Generació</h2>';

        // Formulari per afegir una nova Generació
        $html .= '
            <form method="post">
                <div class="mb-3">
                    <input type="text" class="form-control" name="nom" placeholder="Nom" required>
                </div>
                <button type="submit" name="add_generacio" class="btn btn-success w-100">Afegir Generació</button>
            </form>

            <br><br>

            <!-- Formulari per cercar Generacions -->
            <form method="get" class="mb-3">
                <input type="hidden" name="section" value="generacio">
                <div class="row">
                    <div class="col-md-6 col-lg-4">
                        <div class="input-group">
                            <input type="text" class="form-control" name="search" placeholder="Cerca Generació" value="'.htmlspecialchars($search).'">
            ...
            $html .= "<tr>
                        ...
                        <td>
                            <form method='post' style='display:inline-block;'>
                                ...
                                <button type='submit' name='delete_generacio' class='btn btn-danger'>Eliminar</button>
                            </form>
        ...
        // Formulari per actualitzar una Generació (apareix sota de la taula)
        $html .= '
            <div id="formulario-actualizar" style="display: none; margin-top: 20px;">
                <form method="post">
                    ...
                    <button type="submit" name="update_generacio" class="btn btn-success">Guardar Canvis</button>
        ...
        if ($total_pages > 1) {
            ...
            if ($page > 1) {
                $prev_page = $page - 1;
                $html .= '<li class="page-item"><a class="page-link" href="?section=generacio&search='.urlencode($search).'&page='.$prev_page.'">Anterior</a></li>';
            } else {
            ...
            for ($i = $start_page; $i <= $end_page; $i++) {
                if ($i == $page) {
                    ...
                    $html .= '<li class="page-item"><a class="page-link" href="?section=generacio&search='.urlencode($search).'&page='.$i.'">'.$i.'</a></li>';
                }
            }
            ...
            if ($page < $total_pages) {
                $next_page = $page + 1;
                $html .= '<li class="page-item"><a class="page-link" href="?section=generacio&search='.urlencode($search).'&page='.$next_page.'">Següent</a></li>';
            } else {
