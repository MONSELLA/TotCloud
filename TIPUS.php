<?php
// TIPUS.php
// Classe per gestionar els Tipus de Discs Durs amb paginació i cerca
...
class TIPUS {
    ...
    // Afegir un nou Tipus
    public function addTipus($nom) {
        $stmt = $this->conn->prepare("INSERT INTO TIPUS (nom) VALUES (?)");
    ...
    // Eliminar un Tipus
    public function deleteTipus($nom) {
        $stmt = $this->conn->prepare("DELETE FROM TIPUS WHERE nom = ?");
        $stmt->bind_param("s", $nom);
        $stmt->execute();
        $stmt->close();
    }

    // Actualitzar un Tipus
    public function updateTipus($old_nom, $new_nom) {
        $stmt = $this->conn->prepare("UPDATE TIPUS SET nom = ? WHERE nom = ?");
        $stmt->bind_param("ss", $new_nom, $old_nom);
        $stmt->execute();
        $stmt->close();
    }

    // Obtenir el codi HTML per mostrar els Tipus amb paginació i cerca
    public function getHTML() {
        ...
        $search_param = "%" . $search . "%";
        $stmt = $this->conn->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM TIPUS WHERE nom LIKE ? LIMIT ?, ?");
        ...
        $html = '<h2>Tipus de Disc Dur</h2>';

        // Formulari per afegir un nou Tipus
        $html .= '
            <form method="post">
                <div class="mb-3">
                    <input type="text" class="form-control" name="nom" placeholder="Nom" required>
                </div>
                <button type="submit" name="add_tipus" class="btn btn-success w-100">Afegir Tipus de Disc Dur</button>
            </form>

            <br><br>

            <!-- Formulari per cercar Tipus -->
            <form method="get" class="mb-3">
                <input type="hidden" name="section" value="tipus">
                <div class="row">
                    <div class="col-md-6 col-lg-4">
                        <div class="input-group">
                            <input type="text" class="form-control" name="search" placeholder="Cerca Tipus" value="'.htmlspecialchars($search).'">
            ...
            $html .= "<tr>
                        ...
                        <td>
                            <form method='post' style='display:inline-block;'>
                                ...
                                <button type='submit' name='delete_tipus' class='btn btn-danger'>Eliminar</button>
                            </form>
        ...
        // Formulari per actualitzar un Tipus (apareix sota de la taula)
        $html .= '
            <div id="formulario-actualizar" style="display: none; margin-top: 20px;">
                <form method="post">
                    ...
                    <button type="submit" name="update_tipus" class="btn btn-success">Guardar Canvis</button>
        ...
        if ($total_pages > 1) {
            ...
            if ($page > 1) {
                $prev_page = $page - 1;
                $html .= '<li class="page-item"><a class="page-link" href="?section=tipus&search='.urlencode($search).'&page='.$prev_page.'">Anterior</a></li>';
            } else {
            ...
            for ($i = $start_page; $i <= $end_page; $i++) {
                if ($i == $page) {
                    ...
                    $html .= '<li class="page-item"><a class="page-link" href="?section=tipus&search='.urlencode($search).'&page='.$i.'">'.$i.'</a></li>';
                }
            }
            ...
            if ($page < $total_pages) {
                $next_page = $page + 1;
                $html .= '<li class="page-item"><a class="page-link" href="?section=tipus&search='.urlencode($search).'&page='.$next_page.'">Següent</a></li>';
            } else {
